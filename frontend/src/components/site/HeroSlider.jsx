...
const slideTestId = (s) => `hero-cta-${(s.tag || s.id || "slide").toLowerCase().replace(/\s+/g, '-')}`;
        ...
        <div className="max-w-2xl text-white">
          {s.tag && (
            <div className="inline-flex items-center gap-2 bg-honda px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.2em] mb-5">
              <span className="w-1.5 h-1.5 bg-white"></span>{s.tag}
            </div>
          )}
          ...
          {s.cta && (
            <Link to={s.cta_link || "/bikes"} className="mt-8 inline-flex items-center gap-2 bg-honda px-7 py-4 text-sm font-bold uppercase tracking-wider hover:bg-honda-dark transition-colors" data-testid={slideTestId(s)}>
...
function Dots({ slides, idx, onSelect }) {
...
export default function HeroSlider() {
  ...
  return (
    <section className="relative h-[520px] sm:h-[600px] bg-gray-950 overflow-hidden" data-testid="hero-slider">
      {slides.map((s, i) => <Slide key={s.id || s.tag} slide={s} isActive={i === idx} />)}
      <Dots slides={slides} idx={idx} onSelect={setIdx} />
    </section>
  );
}
